...
void constroiPlane(float lenght, int divisions, const char* file) {
	...
	//printf("comprimento = %f , pontomedio = %f\n", comprimento, pmedio);
	...
	for (int i = 0; i < divisions; i++) {
		for (int j = 0; j < divisions; j++) {
			...
			//printf("1 x %f y 0 z %f, 2 x %f y 0 z %f\n", x1, z1, x2, z2);
...
void constroiSphere(float raio, int slices, int stacks, const char* file) {
	...
	//coordenadas polares: (alpha, raio)
	// ==
	//coordenadas cartesianas: px=raio*sin(alpha) e pz=raio*cos(alpha)

	//coordenadas esfericas: (alpha, beta, raio), com -90<beta<90
	//==
	//coordenadas cartesianas: pz=raio*cos(beta)*cos(alpha); px=raio*cos(beta)*sin(alpha); py=raio*sin(beta)
